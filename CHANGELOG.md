...
- Test file excluded from build output so it is no longer shipped in the npm package
- `typecheck` script now uses `tsconfig.check.json` which includes the test file without emitting

### Added

- `tsconfig.check.json` for full type-checking including the test file
